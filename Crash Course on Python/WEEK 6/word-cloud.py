...
def calculate_frequencies(file_contents):
    ...
    # If u find the punctuations simply replace it by none
    for letter in punctuations:
        file_contents =  file_contents.replace(letter,'')
    
    # If u find the word uninteresting simply replace it 
    for word in uninteresting_words:
        w = ' '+word+' '                  # Adjust the space before and after the word
        file_contents = file_contents.replace(w,' ')
    
    for word in file_contents.split():
        if word.lower() not in taken:
            taken.append(word.lower())     # Add it first time in list
            
            if word not in frequencies:    # if not in the dict for first time add it Once
                frequencies[word] = 1
            else:
                frequencies[word] += 1
